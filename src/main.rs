...
#[repr(C)] // 構造体のメモリレイアウトをC言語の規約に合わせる
...
// x86機械語の一つのHLT命令を定義。CPUを割り込みが来るまで休ませる。
pub fn hlt() {
    // HLT命令
    unsafe { asm!("hlt") }
}
...
trait Bitmap {
    ...
    /// # Safety
    ///
    /// Returned pointer is valid as long s the given coordinates are valid
...
fn draw_point<T: Bitmap>(buf: &mut T, color: u32, x: i64, y: i64) -> Result<()> {
    let pixel = buf.pixel_at_mut(x, y);
    *pixel.ok_or("Out of Range")? = color;
